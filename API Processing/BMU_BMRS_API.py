...
def get_db_date(min_or_max = 'MAX', metric = 'FPN'):
    if metric == 'BOALF':
        date_query = '''SELECT {}(DateTime) FROM BMRS_{}'''.format(min_or_max.upper(), metric)
    else: 
...
def get_boalf_table(ava_df):
    ...
    boalf_df = boalf_df[['recordType', 'bmUnitID', 'bidOfferAcceptanceNumber', 'acceptanceTime', 'timeFrom','bidOfferLevelFrom', 'timeTo', 'bidOfferLevelTo']]
    ...
    boalf_df = boalf_df.rename(columns = {'recordType' : 'record_type',
                                      'bmUnitID' : 'bmunit_id', 
                                      'timeFrom' : 'from_time',
...
#download_start_date = date(2020,7,8)
#
#download_end_date = date(2020,11,22)
...
try:
    ...
    try:
        ...
        bh = bh.drop_duplicates(subset = ['DateTime', 'bmunit_id', 'acceptance_number', 'settlement_period'])
        write_to_db(engine, bh, 'BMRS_BOALF')
    except Exception as e:
        logger.info('Failed to write to DB!')
        logger.info('Error: {}'.format(e))
        sd = str(get_db_date('MAX','BOALF'))
        send_error_mail('Failed to write into BMRS_BOALF!. Error body: {}. Last date in DB is {}. Time now is {}.'.format(e, sd, time.time()))
    # try for the rest 
...
try:
    st = time.time()
    remove_duplicates = '''
                        Delete from [sandbox].[dbo].[BMRS_BOALF] WHERE [id] in 
                        (
                        SELECT max([id])
                        FROM  [sandbox].[dbo].[BMRS_BOALF]
                        GROUP BY [bmunit_id], [acceptance_number], [DateTime], [settlement_period]
                        HAVING Count(*) >1
                        )
                        '''
    ...
    et = time.time()
    logger.info('Removed ' + str(duplicates.rowcount) + ' duplicates from BMRS_BOALF in {:4.2f} seconds.'.format(et-st))
except:
    logger.info('Could not remove duplicates from BOALF.')
    send_error_mail('Could not remove duplicates from BMRS_BOALF. Time now is {}'.format(time.time()))
for metric in ['PN', 'MEL', 'MIL']:
    try:
        st = time.time()
        remove_duplicates = '''
